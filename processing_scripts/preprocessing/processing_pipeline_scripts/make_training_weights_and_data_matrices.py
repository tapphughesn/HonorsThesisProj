...
data_dir = '/proj/NIRAL/users/nic98/DATA/training/processed_data_nifti'
save_dir = '/proj/NIRAL/users/nic98/DATA/training/train_on'
...
while (i < len(data_files)):
    ...
    #cut out "t1w" and "t2w", and the "half1" and "half2" in file names
    data_name = os.path.basename(data_files[i+2])[0:6]
    label_name = data_name + "_label"
    weight_name = data_name + "_weight"

    half1_label = load_nifti(data_files[i])[0]
    half2_label = load_nifti(data_files[i+1])[0]
    half1_t1w = load_nifti(data_files[i+2])[0]
    half2_t1w = load_nifti(data_files[i+3])[0]
