...
boundary_weight_factor = 5.0
calculate_categorical_accuracy = True
glob_learning_rate = 1e-6
...
data_files = sorted(glob("/proj/NIRAL/users/nic98/DATA/pretraining/small_train_on/data_arrays/*"))
label_files = sorted(glob("/proj/NIRAL/users/nic98/DATA/pretraining/small_train_on/label_arrays/*"))
weight_files = sorted(glob("/proj/NIRAL/users/nic98/DATA/pretraining/small_train_on/weight_arrays/*"))
boundary_files = sorted(glob("/proj/NIRAL/users/nic98/DATA/pretraining/small_train_on/boundary_arrays/*"))
...
def train(model, learning_rate, optimizer, features, labels, weights = tf.constant(1.0), boundaries = tf.constant(0.0),
    ...
    with tf.GradientTape() as tape:
        ...
        # weighted_loss_tensor = loss_tensor * weights
        # loss_tensor = class_weights_factor * weighted_loss_tensor + (1 - class_weights_factor) * loss_tensor
...
for epoch in epochs:
    ...
    for batch in range( len(data_files) // batch_size ):
        ...
        for i in range(batch_size):
            rand_index = permuted_indices[batch*batch_size + i]
            print(rand_index)
            data_arrays.append(np.load(data_files[rand_index]))
            label_arrays.append(np.load(label_files[rand_index]))
            weight_arrays.append(np.load(weight_files[rand_index]))
            boundary_arrays.append(np.load(boundary_files[rand_index]))
